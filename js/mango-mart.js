...
const offlineIndicator = {
    ...
    init() {
        ...
        indicator.innerHTML = `
            ...
            <div class="offline-indicator__text">
                <p class="offline-indicator__title">You're offline</p>
    ...
    showOffline() {
        this.init();
        this.setMessage("You're offline", 'We’re trying to reconnect…');
        if (this.el) {
    ...
    showOnline() {
        this.init();
        this.setMessage('Back online', 'Connections restored');
        if (this.el) {
...
function getTrendingTerms() {
    try {
        const productNames = (mockProducts || []).map(p => p.name).slice(0, 50);
        const base = ['Fruits', 'Vegetables', 'Dairy', 'Bakery', 'Pantry', 'Stationary', 'Mangoes', 'Banana', 'Tomatoes', 'Spinach'];
        const categories = (sampleCategories || []).map(c => c.name);
        return Array.from(new Set([...base, ...categories, ...productNames])).slice(0, 20);
    } catch {
...
const db = {
    ...
    async addToUserCart(userId, product) {
        ...
        try {
            // First check if item already exists (use .maybeSingle() to avoid errors when not found)
            ...
            // Check for 406 (Not Acceptable) - table might not exist or RLS blocking
            if (checkError) {
                if (checkError.message?.includes('406') || checkError.status === 406 || checkError.statusCode === 406 || 
                    (checkError.code && checkError.code.toString().includes('406'))) {
                    console.error('⚠️ user_carts table not accessible (406 error). Please run create_user_carts_table.sql in Supabase SQL Editor.');
                    return { 
                        data: null, 
                        error: { 
            ...
            if (existingItem) {
                ...
                if (error) {
                    // Handle duplicate key error (409 Conflict) - item already exists, update instead
                    if (error.code === '23505' || error.message?.includes('duplicate key') || error.message?.includes('unique constraint')) {
                        // Try to fetch and update the existing item
                        ...
                        if (!fetchError && existingItems && existingItems.length > 0) {
                            ...
                            console.error('Could not fetch existing item after duplicate key error:', fetchError);
                            return { data: null, error };
                        }
                    }
    ...
    async updateUserCartItem(userId, productId, quantity) {
        ...
        if (!uuidRegex.test(String(userId).trim())) {
            ...
            if (fetchError) {
                // Check for 406 (Not Acceptable) - table might not exist or RLS blocking
                // Note: PGRST116 is "no rows found" which is OK, not an error
                if (fetchError.message?.includes('406') || fetchError.status === 406 || fetchError.statusCode === 406 || 
                    (fetchError.code && fetchError.code.toString().includes('406'))) {
                    console.error('⚠️ user_carts table not accessible (406 error). Please run create_user_carts_table.sql in Supabase SQL Editor.');
                    return { 
                        data: null, 
                        error: { 
                ...
                if (error) {
                    ...
                    console.log('✅ Successfully deleted cart item from Supabase');
                }
                ...
                if (error) {
                    ...
                    console.log('✅ Successfully updated cart item in Supabase:', data);
                }
...
class CustomerDashboard {
            ...
            main.innerHTML = `
                ...
                <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ${searchResults.map(product => {
                        ...
                        return `
                            <div class="product-card cursor-pointer" data-product-id="${product.id}" data-product-clickable="true">
                                    ...
                                    <div class="h-full min-h-[320px] bg-gradient-to-br ${category.color} p-8 flex flex-col justify-between text-white relative">
                                        ...
                                        <div class="relative z-10">
                                            <div class="flex items-center justify-between mb-6">
                                                ...
                                                <span class="text-sm font-semibold bg-white bg-opacity-25 backdrop-blur-sm px-4 py-2 rounded-full border border-white border-opacity-30">
    ...
    async addToCart(product) {
        ...
        // OPTIMISTIC UPDATE: Update local cart immediately for instant UI feedback
        ...
        // Refresh cart display if on cart page (without delay)
        ...
        (async () => {
            ...
            if (result.error) {
                console.error('Supabase cart error:', result.error);
                    ...
                    // Success - update local cart with Supabase response to ensure sync
                    if (result.data) {
                        const updatedItem = result.data[0] || result.data;
                        const existingItem = this.cart.find(item => item.id === product.id);
                        if (existingItem) {
                            // Use the exact quantity from Supabase to ensure sync (use parseInt to handle string values)
                            existingItem.quantity = parseInt(updatedItem.quantity) || 0;
                        } else if (updatedItem) {
                            // Item was added, add it to local cart
                            this.cart.push({
                                id: updatedItem.product_id || product.id,
                                name: updatedItem.product_name || product.name,
                                price: updatedItem.product_price || product.price,
                                image: updatedItem.product_image || product.image,
                                quantity: parseInt(updatedItem.quantity) || 1
                            });
                        }
                            ...
                            <div class="flex flex-col justify-center">
                                ...
                                <div class="mb-6">
                                    <div class="flex items-baseline space-x-3 mb-4">
                                        <span class="text-4xl sm:text-5xl font-bold text-emerald-600">₹${product.price}</span>
                                        ${stock > 0 ? `
                                            <span class="text-sm text-gray-500">In Stock (${stock} available)</span>
                                        ` : ''}
                                    </div>
                                    ...
                                    ` : `
                                        <div class="flex items-center space-x-4 mb-6" data-product-action="${product.id}">
                                            <template data-template="add">
                                                <button class="add-to-cart-btn flex-1 btn btn-primary text-lg py-3" data-product-id="${product.id}">
                                    ...
                                    return `
                                        <div class="product-card cursor-pointer bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden" data-product-id="${relatedProduct.id}" data-product-clickable="true">
                                            ...
                                            <div class="p-4">
                                                ...
                                                <div class="flex items-center justify-between mb-3">
                                                    <span class="text-emerald-600 font-bold text-lg">₹${relatedProduct.price}</span>
                                                </div>
    ...
    async removeFromCart(productId) {
        ...
        try {
            ...
            // On success, UI is already updated optimistically, no need to reload
        } catch (error) {
                    ...
                    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div class="relative flex items-center justify-center h-16">
                            ...
                            <div class="flex items-center gap-2 sm:gap-3">
                                <img src="assets/mango-logo.svg" alt="Mango Mart logo" class="w-9 h-9 sm:w-10 sm:h-10 drop-shadow-sm">
                            ...
                            <div class="delivery-headline-container">
                                <div class="delivery-headline-text">
                                    <span class="inline-flex items-center">
                                        ...
                                        <span>⚡ Within 1 Hour Delivery Available!</span>
                ...
                <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        ${filteredProducts.map(product => {
                            ...
                            return `
                                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow cursor-pointer" data-product-id="${product.id}" data-product-clickable="true">
                                    <div class="product-image-container"><img src="${product.image}" alt="${product.name}"></div>
                                    <div class="p-4">
                                        <h3 class="font-semibold text-gray-900 mb-2">${product.name}</h3>
                                        <p class="text-gray-600 text-sm mb-3">${product.description}</p>
                                ...
                                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <span class="quantity font-semibold text-gray-900 min-w-[3rem] md:min-w-[2rem] text-center text-lg md:text-base">${item.quantity}</span>
                                            <button class="quantity-btn w-14 h-14 md:w-10 md:h-10 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-all active:scale-95" 
...
class DeliveryDashboard {
    ...
    renderOrders() {
        ...
        ` : this.orders.map(order => `
            <div class="border border-gray-200 rounded-lg p-3 sm:p-4 mb-3 sm:mb-4 hover:shadow-md transition-shadow">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
                    <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                        <div class="text-xs sm:text-sm text-gray-600">
                            <span class="font-medium">Order ID:</span> <span class="break-all">${order.id}</span>
                        </div>
                        <div class="text-xs sm:text-sm text-gray-600">
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3">
                        <span class="px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${this.getDeliveryStatusClass(order.delivery_status)}">
                            ${this.getDeliveryStatusText(order.delivery_status)}
                        </span>
                        <button class="btn btn-primary text-xs sm:text-sm w-full sm:w-auto" onclick="app.currentDashboard.viewOrderDetails('${order.id}')">
                            <i data-lucide="eye" class="w-3 h-3 sm:w-4 sm:h-4 mr-1"></i>
                            View Details
                        </button>
                    </div>
                </div>
                <div class="text-xs sm:text-sm text-gray-600 space-y-1 sm:space-y-0">
                    <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4">
                        <span class="flex items-center"><i data-lucide="phone" class="w-3 h-3 sm:w-4 sm:h-4 inline mr-1"></i> ${order.customer_mobile}</span>
                        <span class="flex items-center"><i data-lucide="map-pin" class="w-3 h-3 sm:w-4 sm:h-4 inline mr-1"></i> <span class="break-words">${order.customer_address}</span></span>
                        <span class="flex items-center"><i data-lucide="calendar" class="w-3 h-3 sm:w-4 sm:h-4 inline mr-1"></i> ${new Date(order.created_at).toLocaleDateString()}</span>
                    </div>
                </div>
        ...
        ` : this.previousOrders.map(order => `
            <div class="border border-gray-200 rounded-lg p-3 sm:p-4 mb-3 sm:mb-4 bg-gray-50">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
                    <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                        <div class="text-xs sm:text-sm text-gray-600">
                            <span class="font-medium">Order ID:</span> <span class="break-all">${order.id}</span>
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3">
                        ...
                        <button class="btn btn-outline text-xs sm:text-sm w-full sm:w-auto" onclick="app.currentDashboard.viewOrderDetails('${order.id}')">
                            View Details
                        </button>
                    </div>
                </div>
                <div class="text-xs sm:text-sm text-gray-600 space-y-1 sm:space-y-0">
                    <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4">
                        ...
                        <span class="flex items-center"><i data-lucide="map-pin" class="w-3 h-3 sm:w-4 sm:h-4 inline mr-1"></i> <span class="break-words">${order.customer_address}</span></span>
                        <span class="flex items-center"><i data-lucide="calendar" class="w-3 h-3 sm:w-4 sm:h-4 inline mr-1"></i> ${new Date(order.created_at).toLocaleDateString()}</span>
                        ${order.delivered_at ? `<span class="flex items-center"><i data-lucide="check-circle" class="w-3 h-3 sm:w-4 sm:h-4 inline mr-1"></i> Delivered: ${new Date(order.delivered_at).toLocaleDateString()}</span>` : ''}
                    </div>
                </div>
                ${this.renderOrderItems(order)}
            </div>
        ...
        return `
            ...
            <div class="mb-6 sm:mb-8">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4 sm:mb-6">
                    <h2 class="text-xl sm:text-2xl font-bold text-gray-900">Active Orders</h2>
                    <span class="px-2 sm:px-3 py-1 bg-blue-100 text-blue-800 text-xs sm:text-sm font-medium rounded-full">${this.orders.length} Active</span>
                </div>
            ...
            <div>
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4 sm:mb-6">
                    <h2 class="text-xl sm:text-2xl font-bold text-gray-900">Previous Orders</h2>
                    <span class="px-2 sm:px-3 py-1 bg-green-100 text-green-800 text-xs sm:text-sm font-medium rounded-full">${this.previousOrders.length} Delivered</span>
                </div>
                <div class="space-y-3 sm:space-y-4">
                    ${previousOrdersHtml}
                </div>
                        ...
                        ${order.items.map((item, index) => `
                            <div class="flex items-center space-x-2 sm:space-x-3 p-2 sm:p-3 bg-gray-50 rounded-lg">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="text-xs sm:text-sm font-medium text-gray-900 truncate">${item.name || 'Product'}</h4>
