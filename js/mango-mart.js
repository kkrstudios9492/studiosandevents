    ...
    async updateUserCartItem(userId, productId, quantity) {
        ...
        if (!uuidRegex.test(String(userId).trim())) {
            ...
            if (fetchError) {
                ...
                if (quantity <= 0) {
                    // Nothing to delete - treat as success silently
                            ...
                            <div class="category-products-row">
                                <div class="category-products-row-inner">
                                    ${row.map(product => {
                                        ...
                                        return `
                                            <div class="category-product-card bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow cursor-pointer" data-product-id="${product.id}" data-product-clickable="true">
                                                <div class="product-image-container"><img src="${product.image}" alt="${product.name}"></div>
                                                <div class="p-4">
                                                    <h3 class="font-semibold text-gray-900 mb-2">${product.name}</h3>
                                                    <p class="text-gray-600 text-sm mb-3">${product.description}</p>
                        ...
                        <div class="space-y-3 sm:space-y-4">
                            ${this.cart.map(item => `
                                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-4">
                                    <div class="flex items-center space-x-4">
                                        <img src="${item.image}" alt="${item.name}" class="w-14 h-14 sm:w-16 sm:h-16 object-cover rounded">
                                        <div class="flex-1">
                                        ...
                                        <div class="flex items-center space-x-3 sm:space-x-4">
                                            <button class="quantity-btn w-12 h-12 sm:w-14 sm:h-14 md:w-10 md:h-10 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-all active:scale-95" 
                                                    data-id="${item.id}" data-action="decrease">
                                                <i data-lucide="minus" class="w-5 h-5 sm:w-6 sm:h-6 md:w-4 md:h-4"></i>
                                            </button>
                                            <span class="quantity font-semibold text-gray-900 min-w-[2.5rem] sm:min-w-[3rem] text-center text-lg md:text-base">${item.quantity}</span>
                                            <button class="quantity-btn w-12 h-12 sm:w-14 sm:h-14 md:w-10 md:h-10 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-all active:scale-95" 
                                                    data-id="${item.id}" data-action="increase">
                                                <i data-lucide="plus" class="w-5 h-5 sm:w-6 sm:h-6 md:w-4 md:h-4"></i>
                                            </button>
                                        </div>
                                    </div>
                                </div>
