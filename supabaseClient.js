...
async function sbUpsertMany(table, rows, conflict){ if (!rows || !rows.length || !window.supabaseClient) return; try{ await window.supabaseClient.from(table).upsert(rows, { onConflict: conflict||'id' }); }catch(e){ console.warn('sbUpsertMany', table, e);} }
async function sbInsertMany(table, rows){ if (!rows || !rows.length || !window.supabaseClient) return; try{ await window.supabaseClient.from(table).insert(rows); }catch(e){ console.warn('sbInsertMany', table, e);} }
