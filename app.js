class MangoMartApp {
  constructor() {
    ...
    // Try to ensure Razorpay SDK is present (dynamic load fallback)
    async ensureRazorpay(){
      if (window.Razorpay) return true;
      if (!document.querySelector('script[data-razorpay-loaded]')){
        try{
          const s = document.createElement('script');
          s.src = 'https://checkout.razorpay.com/v1/checkout.js';
          s.async = true;
          s.setAttribute('data-razorpay-loaded','1');
          document.head.appendChild(s);
        }catch(_){ return false; }
      }
      const start = Date.now();
      while(!window.Razorpay && (Date.now()-start) < 5000){
    ...
    async startPayment() {
      ...
      // Ensure Razorpay is available (attempt dynamic load if needed)
      const hasRazorpay = await this.ensureRazorpay();
      if (!hasRazorpay || !window.RAZORPAY_KEY) {
    ...
    async finalizeOrder(details){
      ...
      // Persist order summary and redirect IMMEDIATELY (use replace to avoid back to checkout)
      ...
      // Post-redirect background enrich and sync
      try {
      ...
      try { setTimeout(()=>{ syncUsersToSupabase().catch(()=>{}).then(()=> syncOrdersToSupabase().catch(()=>{})); }, 0); } catch(_) {}
    }
