class MangoMartApp {
    ...
    login(e) {
      e.preventDefault();
      if (!doLogin(email.value.trim(), password.value.trim())) {
    ...
    openProductDetail(productId){
      ...
      const qty = item ? item.quantity : 0;
      // simple related: same category excluding current, top 4
      ...
      this.updateCustomerProductsVisibility(true);
      detail.innerHTML = `<div style="display:flex;flex-direction:column;gap:.75rem">
          <img src="${p.image}" alt="${p.name}" />
          <div class="meta"><div class="price">₹${p.price}</div><div>Stock: ${p.stockStatus!=='out'?'Available':'Out of Stock'}</div></div>
          <div><strong>Description</strong><div>${p.description}</div></div>
          <div>${controls}</div>
    ...
    renderCustomerOrders(){
      const u = getCurrentUser();
      const all = getOrders().filter(o=>o.customerId===u.id).sort((a,b)=>new Date(b.date)-new Date(a.date));
      ...
      const render = (o)=>{
        ...
        return `<div class="order-card"><div class="order-meta"><div><strong>Order #${o.id}</strong> – ₹${o.total}</div>${stepper}${timeline}</div><div class="order-actions">${actions}</div></div>`;
      };
      curEl.innerHTML = current.length ? current.map(render).join('') : '<p>No current orders</p>';
      prevEl.innerHTML = previous.length ? previous.map(render).join('') : '<p>No previous orders</p>';
    ...
    renderDelivery() {
      ...
      const list = o.filter((x) => x.status !== 'delivered' && (x.status === "pending" || x.deliveryAgentId === u.id));
      document.getElementById("assigned-orders").innerHTML = list
        .map((x) => {
          ...
          return `<div class="cart-item">
            <div style="flex:1;min-width:240px;margin-right:1rem">
              <div><strong>Order #${x.id}</strong> – ₹${x.total}</div>
              <div>${x.customerName || ''} ${contact.mobile?`<span class=\"badge\">${contact.mobile}</span>`:''}</div>
              ${addr?`<div class=\"badge\">${addr}</div>`:''}
              ${thumbList}
              ${stepper}
              ${rating?`<div class=\"rating\">${rating} <span class=\"muted\">${x.rating.feedback?`– ${x.rating.feedback}`:''}</span></div>`:''}
            </div>
            <div class="action-bar" style="min-width:260px">
              <button class="btn btn-outline" onclick="app.viewOrderOnMap('${x.id}')">Map</button>
              <button class="btn btn-outline" onclick="app.viewAddress('${x.id}')">View Address</button>
              <button class="btn btn-outline" ${stepIdx!==0?'disabled':''} onclick="app.flowPickup('${x.id}')">Mark Picked up</button>
              <button class="btn btn-outline" ${stepIdx!==1?'disabled':''} onclick="app.flowOutForDelivery('${x.id}')">Mark Out for delivery</button>
              <button class="btn btn-primary" ${stepIdx!==2?'disabled':''} onclick="app.flowDelivered('${x.id}')">Mark Delivered</button>
            </div>
          </div>`;
        })
        .join("");
      ...
      // delivered history
      const hist = o.filter(x=>x.deliveryAgentId===u.id && x.status==='delivered').sort((a,b)=>new Date(b.date)-new Date(a.date)).slice(0,10);
      const deliveredBox = document.getElementById('delivered-orders');
      if (deliveredBox) {
        deliveredBox.innerHTML = hist.map(h=>{
          ...
          return `<div class=\"cart-item\"><div><div><strong>Order #${h.id}</strong> – ₹${h.total}</div>${thumbList}<div>Delivered on ${new Date(h.date).toLocaleString()}</div><div class=\"rating\">${rating}</div></div></div>`;
        }).join('') || '<p>No delivered orders yet</p>';
      }
    }
