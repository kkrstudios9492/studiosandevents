class MangoMartApp {
  constructor() {
    ...
    // Track orders being updated to prevent race conditions
    _updatingOrderStatus = new Set();
    _temporarilyDisableRefresh = false;
    ...
    start() {
      ...
      if (typeof startOrderStatusMonitoring === 'function') {
    ...
    async loadOrdersFromSupabase(){
      try{
        ...
        // Skip refresh if we have recent updates in progress
        const hasRecentUpdates = Array.from(this._updatingOrderStatus).some(orderId => {
          const order = getOrders().find(o => o.id === orderId);
          if (!order || !order.history || order.history.length === 0) return false;
          const lastUpdate = Math.max(...order.history.map(h => h.ts || 0));
          return (Date.now() - lastUpdate) < 30000; // 30 seconds
        });
        
        if (hasRecentUpdates || this._temporarilyDisableRefresh) {
          console.log('Skipping Supabase refresh due to recent local updates or temporary disable');
          return;
        }
        ...
        // Merge with existing orders, preserving local changes but allowing Supabase updates
        const existingOrders = getOrders();
        const existingIds = new Set(existingOrders.map(o => o.id));
        const newOrders = mapped.filter(o => !existingIds.has(o.id));
        
        // Update existing orders with Supabase data, but preserve local changes if they're more recent
        const updatedExistingOrders = existingOrders.map(existingOrder => {
          const supabaseOrder = mapped.find(s => s.id === existingOrder.id);
          if (!supabaseOrder) return existingOrder;
          
          // Check if local order has recent changes (within last 10 seconds) or is being updated
          const now = Date.now();
          const localLastUpdate = existingOrder.history && existingOrder.history.length > 0 
            ? Math.max(...existingOrder.history.map(h => h.ts || 0))
            : 0;
          const isRecentLocalChange = (now - localLastUpdate) < 10000; // 10 seconds
          const isBeingUpdated = this._updatingOrderStatus.has(existingOrder.id);
          
          // Special handling for delivered orders - always preserve local status if it's delivered
          if (existingOrder.status === 'delivered' && supabaseOrder.status !== 'delivered') {
            console.log(`Preserving delivered status for order ${existingOrder.id} (local: delivered, supabase: ${supabaseOrder.status})`);
            return existingOrder;
          }
          
          if ((isRecentLocalChange || isBeingUpdated) && existingOrder.status !== supabaseOrder.status) {
            // Keep local changes if they're recent or being updated
            console.log(`Preserving local changes for order ${existingOrder.id} (status: ${existingOrder.status}, recent: ${isRecentLocalChange}, updating: ${isBeingUpdated})`);
            return existingOrder;
          } else {
            // Use Supabase data but preserve local history
            console.log(`Using Supabase data for order ${existingOrder.id} (status: ${supabaseOrder.status})`);
            return {
              ...supabaseOrder,
        ...
        const mergedOrders = [...updatedExistingOrders, ...newOrders];
    ...
    async finalizeOrder(details){
      ...
      dropPromise.then(async (drop)=>{
        ...
        saveOrders(updatedOrders);
          ...
          await syncOrdersToSupabaseImmediate([order]);
          console.log(`Order ${order.id} successfully saved to Supabase`);
      ...
      this.updateCart();
          ...
          // Show detailed success message with order information
        const c = document.getElementById("modal-content");
        c.innerHTML = `
            <div class="modal-header">
              <h3><i class="fas fa-check-circle" style="color: #22c55e; margin-right: 0.5rem;"></i>Order Placed Successfully!</h3>
              <button class="modal-close" onclick="app.closeModal()">&times;</button>
            </div>
            <div class="modal-body" style="max-height: 70vh; overflow-y: auto;">
              <div class="order-success-details">
                <div class="order-summary-card" style="background: #f8fafc; border-radius: 12px; padding: 1.5rem; margin-bottom: 1rem; border: 1px solid #e2e8f0;">
                  <h4 style="color: #1e293b; margin: 0 0 1rem 0; display: flex; align-items: center; gap: 0.5rem;">
                    <i class="fas fa-receipt"></i> Order Summary
                  </h4>
                  <div class="order-info-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                    <div>
                      <strong style="color: #64748b;">Order ID:</strong><br>
                      <span style="color: #1e293b; font-family: monospace; font-size: 1.1rem;">#${order.id}</span>
                    </div>
                    <div>
                      <strong style="color: #64748b;">Order Date:</strong><br>
                      <span style="color: #1e293b;">${new Date(order.date).toLocaleString()}</span>
                    </div>
                    <div>
                      <strong style="color: #64748b;">Total Amount:</strong><br>
                      <span style="color: #059669; font-size: 1.2rem; font-weight: bold;">₹${order.total}</span>
                    </div>
                    <div>
                      <strong style="color: #64748b;">Status:</strong><br>
                      <span style="background: #fef3c7; color: #92400e; padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.875rem; text-transform: uppercase;">${order.status}</span>
                    </div>
                  </div>
                </div>

                <div class="customer-details-card" style="background: #f0f9ff; border-radius: 12px; padding: 1.5rem; margin-bottom: 1rem; border: 1px solid #bae6fd;">
                  <h4 style="color: #1e293b; margin: 0 0 1rem 0; display: flex; align-items: center; gap: 0.5rem;">
                    <i class="fas fa-user"></i> Delivery Details
                  </h4>
                  <div class="customer-info" style="display: grid; gap: 0.5rem;">
                    <div><strong>Name:</strong> ${order.contact?.name || order.customerName || 'N/A'}</div>
                    <div><strong>Mobile:</strong> ${order.contact?.mobile || 'N/A'}</div>
                    ${order.contact?.altMobile ? `<div><strong>Alt Mobile:</strong> ${order.contact.altMobile}</div>` : ''}
                    <div><strong>Address:</strong> ${order.contact?.address || 'N/A'}</div>
                    <div><strong>Landmark:</strong> ${order.contact?.landmark || 'N/A'}</div>
                    <div><strong>Pincode:</strong> ${order.contact?.pincode || 'N/A'}</div>
                  </div>
                </div>

                <div class="items-details-card" style="background: #fefce8; border-radius: 12px; padding: 1.5rem; margin-bottom: 1rem; border: 1px solid #fde047;">
                  <h4 style="color: #1e293b; margin: 0 0 1rem 0; display: flex; align-items: center; gap: 0.5rem;">
                    <i class="fas fa-shopping-bag"></i> Ordered Items (${order.items.length})
                  </h4>
                  <div class="items-list" style="display: flex; flex-direction: column; gap: 0.75rem;">
                    ${order.items.map(item => `
                      <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.75rem; background: white; border-radius: 8px; border: 1px solid #e5e7eb;">
                        <div>
                          ...
                          <div style="color: #64748b; font-size: 0.875rem;">₹${item.price} × ${item.quantity}</div>
                        </div>
                        <div style="font-weight: bold; color: #059669;">₹${item.price * item.quantity}</div>
                      </div>
                    `).join('')}
                  </div>
                  <div style="border-top: 2px solid #e5e7eb; margin-top: 1rem; padding-top: 1rem; display: flex; justify-content: space-between; align-items: center;">
                    <strong style="font-size: 1.1rem; color: #1e293b;">Total Amount:</strong>
                    <strong style="font-size: 1.3rem; color: #059669;">₹${order.total}</strong>
                  </div>
                </div>

                <div class="next-steps-card" style="background: #f0fdf4; border-radius: 12px; padding: 1.5rem; border: 1px solid #bbf7d0;">
                  <h4 style="color: #1e293b; margin: 0 0 1rem 0; display: flex; align-items: center; gap: 0.5rem;">
                  ...
                  <div style="color: #166534; line-height: 1.6;">
                    ...
                    <p>🚚 A delivery agent will be assigned to your order soon</p>
                    <p>📱 You'll receive real-time updates on your order status</p>
                    <p>📍 You can track your order in real-time once delivery starts</p>
                  </div>
                </div>
              </div>
            </div>
            <div class="modal-footer" style="display: flex; gap: 0.5rem; justify-content: space-between;">
              <button class="btn btn-outline" onclick="app.closeModal(); app.switchTab('products');">
                <i class="fas fa-shopping-cart"></i> Continue Shopping
              </button>
              <button class="btn btn-primary" onclick="app.closeModal(); app.switchTab('orders'); app.renderCustomerOrders();">
          ...
          // Show error message but still add to local storage
          const existingOrders = getOrders();
          const updatedOrders = [...existingOrders, order];
          saveOrders(updatedOrders);
          ...
          c.innerHTML = `
            <div class="modal-header">
              <h3><i class="fas fa-exclamation-triangle" style="color: #f59e0b; margin-right: 0.5rem;"></i>Order Placed (Offline)</h3>
              <button class="modal-close" onclick="app.closeModal()">&times;</button>
            </div>
            <div class="modal-body" style="max-height: 70vh; overflow-y: auto;">
              <div class="order-success-details">
                ...
                <div class="offline-notice-card" style="background: #fef2f2; border-radius: 12px; padding: 1.5rem; border: 1px solid #fecaca;">
                  <h4 style="color: #dc2626; margin: 0 0 1rem 0; display: flex; align-items: center; gap: 0.5rem;">
                    <i class="fas fa-exclamation-triangle"></i> Offline Notice
                  </h4>
                  <div style="color: #dc2626; line-height: 1.6;">
                    <p>⚠️ Your order has been saved locally but couldn't sync to our servers.</p>
                    <p>📱 The order will automatically sync when your connection is restored.</p>
                    <p>✅ You can still view and track your order in the meantime.</p>
                  </div>
                </div>
              </div>
            </div>
    ...
    renderCustomerOrders(){
      ...
      console.log('Current order statuses:', current.map(o => ({id: o.id, status: o.status})));
      console.log('Previous order statuses:', previous.map(o => ({id: o.id, status: o.status})));
      // apply filters
      ...
      const render = (o)=>{
        ...
        const statusIndicator = stepIdx < 3 ? 
          `<div class="status-update-indicator" style="display: flex; align-items: center; gap: 0.5rem; margin-top: 0.5rem; padding: 0.5rem; background: #f0f9ff; border-radius: 8px; border-left: 4px solid #0ea5e9;">
            <div class="status-dot active" style="animation: pulse 2s infinite;"></div>
            <span style="font-size: 0.9rem; color: #0369a1;">Live tracking active</span>
          </div>` : 
          stepIdx === 3 ? 
          `<div class="delivery-complete-indicator" style="display: flex; align-items: center; gap: 0.5rem; margin-top: 0.5rem; padding: 0.5rem; background: #f0fdf4; border-radius: 8px; border-left: 4px solid #22c55e;">
            <div class="status-dot done" style="background: #22c55e;"></div>
            <span style="font-size: 0.9rem; color: #166534;">Order delivered successfully</span>
          </div>` : '';
        ...
        // Render ordered products - always show if order has items
        const products = dedupedItems.length ? dedupedItems.map(item => {
          const product = getProducts().find(p => p.id === item.productId);
          if (!product) {
            // Fallback if product not found - show item details anyway
            return `<div class="order-product-item">
              <div class="order-product-details">
                <div class="order-product-name">${item.productName || 'Product'}</div>
    ...
    trackOrder(orderId){
      ...
      // Store the tracking order ID for real-time updates
      ...
      this.customerTrackInterval = setInterval(()=>{
        ...
        if (order.status==='delivered') { 
          clearInterval(this.customerTrackInterval);
          this._trackingOrderId = null; // Clear tracking order ID when delivered
        }
      }, 5000);
    }
    ...
    renderAdmin() {
      ...
      if (ro) {
        ...
        // Show/hide delete all orders button based on whether there are orders
        const deleteBtn = document.querySelector('.delete-all-orders-btn');
        if (deleteBtn) {
          deleteBtn.style.display = getOrders().length > 0 ? 'flex' : 'none';
        }
      }
    ...
    renderDelivery() {
      ...
      // Show ALL orders from table editor (not just assigned ones)
      const allActiveOrders = o.filter((x) => {
        // Show all orders that are not delivered
        if (x.status === 'delivered') return false;
        // Must have a valid status
        if (!x.status || x.status === '') return false;
        return true;
      });
      
      const allCompletedOrders = o.filter((x) => x.status === 'delivered');
      const todayDeliveries = allCompletedOrders.filter(x => {
        const today = new Date();
        const orderDate = new Date(x.date);
        return orderDate.toDateString() === today.toDateString();
      });
      
      // Count orders assigned to current delivery agent for stats
      const myActiveOrders = o.filter((x) => {
        if (x.deliveryAgentId !== u.id) return false;
        if (x.status === 'delivered') return false;
        if (!x.status || x.status === '') return false;
        return true;
      });
      const myCompletedOrders = o.filter((x) => x.status === 'delivered' && x.deliveryAgentId === u.id);
      
      document.getElementById('delivery-active-count').textContent = myActiveOrders.length;
      document.getElementById('delivery-completed-count').textContent = myCompletedOrders.length;
      document.getElementById('delivery-today-count').textContent = todayDeliveries.length;
      
      // Show ALL orders from table editor
      const list = allActiveOrders;
      const products = getProducts();
      ...
      document.getElementById("assigned-orders").innerHTML = list.length > 0 ? list
        .map((x) => {
          ...
          // Generate comprehensive product list with images and details
              ...
              return `
                <div class="delivery-product-item">
                  <div class="delivery-product-details">
                    <div class="delivery-product-name">${item.productName || 'Product'}</div>
                    <div class="delivery-product-meta">₹${item.price} × ${item.quantity} = ₹${totalPrice}</div>
                  </div>
                </div>
              `;
            }
            return `
              <div class="delivery-product-item">
                ...
                <div class="delivery-product-details">
                  ...
                  <div class="delivery-product-meta">₹${item.price} × ${item.quantity} = ₹${totalPrice}</div>
                  <div class="delivery-product-category">${product.category || 'General'}</div>
                </div>
              </div>
            `;
          }).join('');
          ...
          // Use the best available name: customerName, contact.name, or fallback
          const displayName = x.customerName || x.contact?.name || 'Customer';
          
          return `<div class="delivery-order-card" data-status="${s}" style="border: 2px solid ${x.deliveryAgentId === u.id ? '#22c55e' : x.deliveryAgentId ? '#f59e0b' : '#ef4444'};">
            ...
            <div style="background: ${x.deliveryAgentId === u.id ? '#f0fdf4' : x.deliveryAgentId ? '#fef3c7' : '#fef2f2'}; padding: 0.5rem; margin: 0.5rem 0; border-radius: 4px; border-left: 4px solid ${x.deliveryAgentId === u.id ? '#22c55e' : x.deliveryAgentId ? '#f59e0b' : '#ef4444'};">
              <strong>Assignment Status:</strong> ${x.deliveryAgentId === u.id ? '✅ ASSIGNED TO YOU' : x.deliveryAgentId ? `⚠️ ASSIGNED TO OTHER AGENT (${x.deliveryAgentId})` : '❌ NOT ASSIGNED'}
            </div>
            
            <!-- Enhanced Order Summary -->
            <div class="delivery-order-summary-enhanced" style="background: #f8fafc; border-radius: 12px; padding: 1rem; margin-bottom: 1rem; border: 1px solid #e2e8f0;">
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 0.75rem;">
                <div>
                  <strong style="color: #64748b; font-size: 0.875rem;">Order Date:</strong><br>
                  <span style="color: #1e293b; font-weight: 500;">${new Date(x.date).toLocaleString()}</span>
                </div>
                <div>
                  <strong style="color: #64748b; font-size: 0.875rem;">Total Amount:</strong><br>
                  <span style="color: #059669; font-size: 1.2rem; font-weight: bold;">₹${x.total}</span>
                </div>
              </div>
              <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 0.75rem; border-top: 1px solid #e5e7eb;">
                <div style="display: flex; gap: 1rem;">
                  <div style="text-align: center;">
                    <div style="font-weight: 600; color: #1e293b;">${(x.items||[]).length}</div>
                    <div style="font-size: 0.75rem; color: #64748b;">Products</div>
                  </div>
                  <div style="text-align: center;">
                    <div style="font-weight: 600; color: #1e293b;">${(x.items||[]).reduce((sum, item) => sum + item.quantity, 0)}</div>
                    <div style="font-size: 0.75rem; color: #64748b;">Items</div>
                  </div>
                </div>
                <div style="background: ${s === 'pending' ? '#fef3c7' : s === 'picked_up' ? '#dbeafe' : s === 'out_for_delivery' ? '#d1fae5' : '#f3e8ff'}; color: ${s === 'pending' ? '#92400e' : s === 'picked_up' ? '#1e40af' : s === 'out_for_delivery' ? '#065f46' : '#7c3aed'}; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.75rem; font-weight: 600; text-transform: uppercase;">
                  ${s.replace('_', ' ')}
                </div>
              </div>
            </div>
            ...
            <div class="delivery-order-details">
              ...
              <div class="delivery-detail-item">
                ...
                <span>${x.contact.altMobile}</span>
              </div>
              ` : ''}
            </div>
            <!-- Enhanced Ordered Products Section -->
            <div class="delivery-order-products-enhanced" style="background: #fefce8; border-radius: 12px; padding: 1rem; margin: 1rem 0; border: 1px solid #fde047;">
              <h4 style="color: #1e293b; margin: 0 0 0.75rem 0; display: flex; align-items: center; gap: 0.5rem;">
                <i class="fas fa-shopping-bag"></i> Ordered Products (${(x.items || []).length})
              </h4>
              <div class="delivery-products-list" style="display: flex; flex-direction: column; gap: 0.75rem;">
                ${(x.items || []).map(item => `
                  <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.75rem; background: white; border-radius: 8px; border: 1px solid #e5e7eb;">
                    <div style="display: flex; align-items: center; gap: 0.75rem;">
                      ${productMap[item.productId] ? `<img src="${productMap[item.productId].image}" alt="${item.productName}" style="width: 40px; height: 40px; object-fit: cover; border-radius: 6px;">` : ''}
                      <div>
                        <div style="font-weight: 600; color: #1e293b;">${item.productName}</div>
                        <div style="color: #64748b; font-size: 0.875rem;">₹${item.price} × ${item.quantity}</div>
                        ${productMap[item.productId]?.category ? `<div style="color: #059669; font-size: 0.75rem; background: #f0fdf4; padding: 0.125rem 0.375rem; border-radius: 4px; display: inline-block; margin-top: 0.25rem;">${productMap[item.productId].category}</div>` : ''}
                      </div>
                    </div>
                    <div style="font-weight: bold; color: #059669; font-size: 1rem;">₹${item.price * item.quantity}</div>
                  </div>
                `).join('')}
              </div>
              <div style="border-top: 2px solid #e5e7eb; margin-top: 1rem; padding-top: 1rem; display: flex; justify-content: space-between; align-items: center;">
                <div style="display: flex; gap: 1rem; align-items: center;">
                  ...
                  <div style="font-size: 0.875rem; color: #64748b;">
                    ${(x.items || []).reduce((sum, item) => sum + item.quantity, 0)} items • ${(x.items || []).length} products
                  </div>
                </div>
                <strong style="font-size: 1.3rem; color: #059669;">₹${x.total}</strong>
              </div>
            </div>
            <!-- Enhanced Delivery Address Section -->
            <div class="delivery-order-address-enhanced" style="background: #f0f9ff; border-radius: 12px; padding: 1rem; margin: 1rem 0; border: 1px solid #bae6fd;">
              <h4 style="color: #1e293b; margin: 0 0 0.75rem 0; display: flex; align-items: center; gap: 0.5rem;">
                <i class="fas fa-map-marker-alt"></i> Delivery Address
              </h4>
              <div class="address-details" style="display: grid; gap: 0.5rem;">
                <div><strong>Name:</strong> ${displayName}</div>
                <div><strong>Mobile:</strong> ${x.contact?.mobile || 'N/A'}</div>
                ${x.contact?.altMobile ? `<div><strong>Alt Mobile:</strong> ${x.contact.altMobile}</div>` : ''}
                <div><strong>Address:</strong> ${x.contact?.address || 'N/A'}</div>
                <div><strong>Landmark:</strong> ${x.contact?.landmark || 'N/A'}</div>
                <div><strong>Pincode:</strong> ${x.contact?.pincode || 'N/A'}</div>
              </div>
              ${addr ? `<div style="margin-top: 0.75rem; padding: 0.75rem; background: white; border-radius: 8px; border-left: 4px solid #3b82f6;">
                <strong style="color: #1e40af;">Complete Address:</strong><br>
                <span style="color: #1e293b;">${addr}</span>
              </div>` : ''}
            </div>
            <!-- Order Status Timeline -->
            <div class="delivery-order-timeline" style="background: #f0fdf4; border-radius: 12px; padding: 1rem; margin: 1rem 0; border: 1px solid #bbf7d0;">
              <h4 style="color: #1e293b; margin: 0 0 0.75rem 0; display: flex; align-items: center; gap: 0.5rem;">
                <i class="fas fa-clock"></i> Order Timeline
              </h4>
              <div class="timeline-steps" style="display: flex; justify-content: space-between; align-items: center; position: relative;">
                <div class="timeline-step ${s === 'pending' ? 'active' : s === 'picked_up' || s === 'out_for_delivery' || s === 'delivered' ? 'completed' : ''}" style="text-align: center; flex: 1;">
                  <div style="width: 32px; height: 32px; border-radius: 50%; background: ${s === 'pending' ? '#3b82f6' : s === 'picked_up' || s === 'out_for_delivery' || s === 'delivered' ? '#22c55e' : '#e5e7eb'}; color: white; display: flex; align-items: center; justify-content: center; margin: 0 auto 0.5rem; font-size: 0.75rem;">
                    ${s === 'pending' ? '1' : '✓'}
                  </div>
                  <div style="font-size: 0.75rem; font-weight: 600; color: ${s === 'pending' ? '#3b82f6' : s === 'picked_up' || s === 'out_for_delivery' || s === 'delivered' ? '#22c55e' : '#64748b'};">Order Placed</div>
                  <div style="font-size: 0.625rem; color: #64748b;">${new Date(x.date).toLocaleString()}</div>
                </div>
                <div class="timeline-step ${s === 'picked_up' ? 'active' : s === 'out_for_delivery' || s === 'delivered' ? 'completed' : ''}" style="text-align: center; flex: 1;">
                  <div style="width: 32px; height: 32px; border-radius: 50%; background: ${s === 'picked_up' ? '#3b82f6' : s === 'out_for_delivery' || s === 'delivered' ? '#22c55e' : '#e5e7eb'}; color: white; display: flex; align-items: center; justify-content: center; margin: 0 auto 0.5rem; font-size: 0.75rem;">
                    ${s === 'picked_up' ? '2' : s === 'out_for_delivery' || s === 'delivered' ? '✓' : '2'}
                  </div>
                  <div style="font-size: 0.75rem; font-weight: 600; color: ${s === 'picked_up' ? '#3b82f6' : s === 'out_for_delivery' || s === 'delivered' ? '#22c55e' : '#64748b'};">Picked Up</div>
                  <div style="font-size: 0.625rem; color: #64748b;">Ready for delivery</div>
                </div>
                <div class="timeline-step ${s === 'out_for_delivery' ? 'active' : s === 'delivered' ? 'completed' : ''}" style="text-align: center; flex: 1;">
                  <div style="width: 32px; height: 32px; border-radius: 50%; background: ${s === 'out_for_delivery' ? '#3b82f6' : s === 'delivered' ? '#22c55e' : '#e5e7eb'}; color: white; display: flex; align-items: center; justify-content: center; margin: 0 auto 0.5rem; font-size: 0.75rem;">
                    ${s === 'out_for_delivery' ? '3' : s === 'delivered' ? '✓' : '3'}
                  </div>
                  <div style="font-size: 0.75rem; font-weight: 600; color: ${s === 'out_for_delivery' ? '#3b82f6' : s === 'delivered' ? '#22c55e' : '#64748b'};">Out for Delivery</div>
                  <div style="font-size: 0.625rem; color: #64748b;">On the way</div>
                </div>
                <div class="timeline-step ${s === 'delivered' ? 'completed' : ''}" style="text-align: center; flex: 1;">
                  <div style="width: 32px; height: 32px; border-radius: 50%; background: ${s === 'delivered' ? '#22c55e' : '#e5e7eb'}; color: white; display: flex; align-items: center; justify-content: center; margin: 0 auto 0.5rem; font-size: 0.75rem;">
                    ${s === 'delivered' ? '✓' : '4'}
                  </div>
                  <div style="font-size: 0.75rem; font-weight: 600; color: ${s === 'delivered' ? '#22c55e' : '#64748b'};">Delivered</div>
                  <div style="font-size: 0.625rem; color: #64748b;">Completed</div>
                </div>
              </div>
            </div>
            
            <div class="delivery-order-actions">
              ${x.deliveryAgentId === u.id ? `
                ${s === 'pending' ? `<button class="delivery-action-btn primary" onclick="app.handlePickup('${x.id}')" style="background: #ff0000; color: white;">
                  <i class="fas fa-hand-holding"></i> Pick Up Order
                </button>` : ''}
                ${s === 'picked_up' ? `<button class="delivery-action-btn primary" onclick="app.handleOutForDelivery('${x.id}')" style="background: #00ff00; color: white;">
                  <i class="fas fa-truck"></i> Start Delivery
                </button>` : ''}
                ${s === 'out_for_delivery' ? `<button class="delivery-action-btn primary" onclick="app.handleDelivered('${x.id}')" style="background: #0000ff; color: white;">
                  <i class="fas fa-check-circle"></i> Mark Delivered
                </button>` : ''}
              ` : x.deliveryAgentId ? `
                <button class="delivery-action-btn secondary" onclick="app.assignOrderToMe('${x.id}')" style="background: #f59e0b; color: white;">
                  <i class="fas fa-user-plus"></i> Take Over Order
                </button>
              ` : `
                <button class="delivery-action-btn primary" onclick="app.assignOrderToMe('${x.id}')" style="background: #059669; color: white;">
          ...
          <p>No orders are currently available. Click "Load All Orders" to fetch orders from the Supabase table editor.</p>
          <button class="btn btn-primary" onclick="app.loadAllOrdersFromSupabase()" style="margin-top: 1rem;">
            <i class="fas fa-download"></i> Load All Orders
          </button>
        </div>`;

      // Delivered orders - show all delivered orders
      const delivered = allCompletedOrders;
      
      // Show/hide buttons based on orders
      const clearBtn = document.querySelector('.clear-history-btn');
      if (clearBtn) {
        clearBtn.style.display = delivered.length > 0 ? 'flex' : 'none';
      }
      ...
      document.getElementById("delivered-orders").innerHTML = delivered.length > 0 ? delivered
        ...
        .map((x) => {
          ...
          // Generate comprehensive product list with images for delivered orders
          const productList = (x.items || []).map(item => {
            const product = productMap[item.productId];
            const totalPrice = item.price * item.quantity;
            if (!product) {
          ...
          return `<div class="delivery-order-card">
            ...
            <div class="delivery-order-details">
              <div class="delivery-detail-item">
                <i class="fas fa-user"></i>
                <span>${x.customerName || x.contact?.name || 'Customer'}</span>
              </div>
              <div class="delivery-detail-item">
            ...
            <div class="delivery-order-products">
              <strong>Delivered Products (${(x.items || []).length} items):</strong>
              <div class="delivery-products-list">
                ${productList}
              </div>
              <div class="delivery-order-summary" style="margin-top: 1rem; padding: 0.75rem; background: #f0fdf4; border-radius: 8px; border-left: 4px solid #22c55e;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                  <span style="font-weight: 600; color: #1e293b;">Order Total:</span>
                  <span style="font-weight: 700; color: #166534; font-size: 1.1rem;">₹${x.total}</span>
                </div>
                <div style="font-size: 0.85rem; color: #64748b; margin-top: 0.25rem;">
                  ${(x.items || []).reduce((sum, item) => sum + item.quantity, 0)} items • ${(x.items || []).length} products • Delivered
                </div>
                <div class="thumb-list" style="margin-top: 0.5rem; display: flex; gap: 0.5rem; flex-wrap: wrap;">
                  ${(x.items || []).map(item => {
                    const product = productMap[item.productId];
                    return product ? `<img src="${product.image}" alt="${product.name}" class="thumb" title="${product.name} (${item.quantity})">` : '';
                  }).join('')}
                </div>
              </div>
            </div>
    ...
    initDeliveryMap() {
      ...
      try {
        this.deliveryMap = new google.maps.Map(el, { 
      ...
      this.refreshDeliveryMapMarkers();
        ...
        el.innerHTML = `
          <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; background: #f3f4f6; border-radius: 8px; padding: 2rem; text-align: center;">
            <i class="fas fa-map-marked-alt" style="font-size: 3rem; color: #9ca3af; margin-bottom: 1rem;"></i>
            <h3 style="color: #374151; margin-bottom: 0.5rem;">Map Unavailable</h3>
            <p style="color: #6b7280; margin: 0;">Google Maps requires billing to be enabled. Order tracking will work without the map.</p>
          </div>
        `;
      }
    }
    ...
    refreshDeliveryMapMarkers() {
      ...
      const orders = getOrders().filter(x=>x.status!=="delivered" && x.deliveryAgentId===u.id);
      orders.forEach(o=>{
    ...
    clearDeliveryHistory() {
      ...
      if (!confirm('Are you sure you want to clear all delivery history? This action cannot be undone.')) {
        return;
      }
      
      try {
        // Get all orders and filter out delivered orders for this delivery agent
        const allOrders = getOrders();
        const nonDeliveredOrders = allOrders.filter(order => 
          !(order.status === 'delivered' && order.deliveryAgentId === u.id)
        );
        
        // Update orders in local storage
        setLS('orders', nonDeliveredOrders);
        
        // Sync to Supabase
        saveOrders(nonDeliveredOrders).then(() => {
          console.log('Delivery history cleared successfully');
          
          // Refresh the delivery interface
          this.renderDelivery();
          
          // Show success message
          alert('Delivery history cleared successfully!');
        }).catch(error => {
          console.error('Failed to clear delivery history:', error);
          alert('Failed to clear delivery history. Please try again.');
        });
        
      } catch (error) {
        console.error('Error clearing delivery history:', error);
        alert('An error occurred while clearing delivery history.');
      }
    }

    async loadAllOrdersFromSupabase() {
      try {
        if (!window.__sb || !window.supabaseClient) {
        ...
        loadingMsg.innerHTML = '<div style="padding: 1rem; text-align: center;"><i class="fas fa-spinner fa-spin"></i> Loading all orders from Supabase...</div>';
        document.getElementById("assigned-orders").appendChild(loadingMsg);
        
        // Get ALL orders from Supabase (not just assigned ones)
        ...
        if (error) {
          ...
          alert('Failed to load orders from Supabase: ' + error.message);
          loadingMsg.remove();
          return;
        }
        
        if (!supabaseOrders || supabaseOrders.length === 0) {
          alert('No orders found in Supabase');
          loadingMsg.remove();
          return;
        }
        ...
        const allOrders = supabaseOrders.map(o => ({
          id: o.id,
          customerId: o.customer_id,
          customerName: userMap[o.customer_id] || (o.contact && o.contact.name) || 'Unknown Customer',
        ...
        alert(`Loaded ${allOrders.length} orders from Supabase successfully!`);
    ...
    assignOrderToMe(orderId) {
      ...
      try {
        const o = getOrders();
        const orderIndex = o.findIndex(x => x.id === orderId);
        if (orderIndex === -1) {
          alert('Order not found');
          return;
        }
        
        o[orderIndex].deliveryAgentId = u.id;
        o[orderIndex].status = o[orderIndex].status || 'pending';
        
        setLS('orders', o);
        saveOrders(o);
        
        // Refresh delivery interface
        this.renderDelivery();
        
        alert(`Order ${orderId} assigned to you successfully!`);
    ...
    deleteAllOrders() {
      const u = getCurrentUser();
      if (!u || u.role !== 'admin') {
        alert('Only administrators can delete all orders.');
        return;
      }
      
      // Double confirmation for this critical action
      if (!confirm('⚠️ WARNING: This will delete ALL orders from the system!\n\nThis action cannot be undone and will affect all customers and delivery agents.\n\nAre you absolutely sure you want to proceed?')) {
        return;
      }
      
      // Second confirmation with different wording
      if (!confirm('Final confirmation: Delete ALL orders?\n\nType "DELETE" in your mind and click OK to proceed, or Cancel to abort.')) {
        return;
      }
      ...
      try {
        ...
        // Clear all order items from Supabase
        if (window.__sb && window.supabaseClient) {
          // Delete all order items first (due to foreign key constraints)
          window.supabaseClient.from('order_items').delete().neq('id', 0).then(() => {
            console.log('All order items deleted from Supabase');
            
            // Then delete all orders
            return window.supabaseClient.from('orders').delete().neq('id', '');
          }).then(() => {
            ...
            // Show success message
            alert('✅ All orders have been successfully deleted from the system!');
            
          }).catch(error => {
            console.error('Failed to delete orders from Supabase:', error);
            alert('⚠️ Orders cleared locally but failed to sync with server. Please check your connection.');
          });
        } else {
          // If no Supabase connection, just update locally
          this.updateAdminStats();
          this.renderAdmin();
          alert('✅ All orders have been cleared from local storage!');
        }
    ...
    // Button handler with better error handling
    handlePickup(orderId) {
      console.log(`Pickup button clicked for order ${orderId}`);
      alert(`Pickup button clicked for order ${orderId}`); // Debug alert
      this.flowPickup(orderId).catch(error => {
        console.error(`Failed to pickup order ${orderId}:`, error);
        alert(`Failed to pickup order: ${error.message}`);
      });
    }

    handleOutForDelivery(orderId) {
      console.log(`Out for delivery button clicked for order ${orderId}`);
      alert(`Out for delivery button clicked for order ${orderId}`); // Debug alert
      
        // Force immediate UI refresh before processing
        this.renderDelivery();
        
        // Disable periodic refresh temporarily
        this._temporarilyDisableRefresh = true;
        
        this.flowOutForDelivery(orderId).then(() => {
          // Force UI refresh after successful update
          setTimeout(() => {
            this.renderDelivery();
            console.log(`Post-update UI refresh for order ${orderId}`);
          }, 100);
          
          // Re-enable periodic refresh after a delay
          setTimeout(() => {
            this._temporarilyDisableRefresh = false;
            console.log(`Re-enabled periodic refresh after status update for order ${orderId}`);
          }, 10000); // 10 seconds delay
          
        }).catch(error => {
          console.error(`Failed to mark order ${orderId} as out for delivery:`, error);
          alert(`Failed to update order status: ${error.message}`);
          // Re-enable refresh even on error
          this._temporarilyDisableRefresh = false;
        });
    }
    ...
    handleDelivered(orderId) {
      ...
      alert(`Delivered button clicked for order ${orderId}`); // Debug alert
      this.flowDelivered(orderId).catch(error => {
    ...
    async flowPickup(orderId){
      try {
        console.log(`Starting pickup process for order ${orderId}`);
        
        // Mark this order as being updated to prevent race conditions
        this._updatingOrderStatus.add(orderId);
        
        const o=getOrders().find(x=>x.id===orderId); 
        if(!o|| (o.status&&o.status!=='pending')) {
          console.log(`Order ${orderId} not found or not pending. Current status: ${o?.status}`);
          this._updatingOrderStatus.delete(orderId);
          return;
        }
        const u=getCurrentUser(); 
        if (!u) {
        ...
        if (!o.deliveryAgentId) {
          console.log(`Assigning order ${orderId} to agent ${u.id}`);
          await assignOrderToAgent(orderId, u.id);
        }
        
        // Update order status (this will trigger real-time updates)
        console.log(`Updating order ${orderId} status to picked_up`);
        ...
        console.log(`Order ${orderId} successfully picked up by delivery agent ${u.id}`);
      } catch (error) {
        console.error(`Error in flowPickup for order ${orderId}:`, error);
        throw error; // Re-throw for button handler
      } finally {
        // Remove from updating set after a delay to ensure UI updates
        setTimeout(() => {
          this._updatingOrderStatus.delete(orderId);
    ...
    async flowOutForDelivery(orderId){
      try {
        console.log(`Starting out for delivery process for order ${orderId}`);
        const o=getOrders().find(x=>x.id===orderId); 
        if(!o|| o.status!=='picked_up') {
          console.log(`Order ${orderId} not found or not picked up. Current status: ${o?.status}`);
          return;
        }
        const u=getCurrentUser(); 
        if (!u) {
        ...
        // Force immediate UI refresh multiple times to ensure it updates
        this.renderDelivery(); 
        this.refreshDeliveryMapMarkers();
        
        // Force multiple UI refreshes to ensure status change is visible
        setTimeout(() => {
          this.renderDelivery();
          console.log(`UI refreshed - Order ${orderId} should now show as out_for_delivery`);
        }, 50);
        
        setTimeout(() => {
          this.renderDelivery();
          console.log(`Final UI refresh - Order ${orderId} status check`);
        }, 200);
        
        console.log(`Order ${orderId} successfully marked as out for delivery`);
      } catch (error) {
        console.error(`Error in flowOutForDelivery for order ${orderId}:`, error);
        throw error; // Re-throw for button handler
      }
    }
    ...
    async flowDelivered(orderId){
      try {
        console.log(`Starting delivered process for order ${orderId}`);
        const o=getOrders().find(x=>x.id===orderId); 
        if(!o|| o.status!=='out_for_delivery') {
          console.log(`Order ${orderId} not found or not out for delivery. Current status: ${o?.status}`);
          return;
        }
        const u=getCurrentUser(); 
        if (!u) {
        ...
        // Force immediate sync to Supabase to ensure delivered status is persisted
        try {
          const updatedOrder = getOrders().find(x => x.id === orderId);
          if (updatedOrder) {
            await syncSingleOrderToSupabase(updatedOrder);
            console.log(`Order ${orderId} delivered status force-synced to Supabase`);
          }
        } catch (error) {
          console.error(`Failed to force sync delivered status for order ${orderId}:`, error);
        }
        ...
        // Force refresh customer orders to move delivered order to history
        if (o.customerId) {
          const customer = getUsers().find(u => u.id === o.customerId);
          if (customer && customer.role === 'customer') {
            this.renderCustomerOrders();
          }
        }
        
        console.log(`Order ${orderId} successfully marked as delivered and moved to history`);
      } catch (error) {
        console.error(`Error in flowDelivered for order ${orderId}:`, error);
        throw error; // Re-throw for button handler
      }
    }
