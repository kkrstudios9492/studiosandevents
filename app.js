  ...
  constructor() {
    ...
    async startPayment() {
      ...
      if (!window.Razorpay || !window.RAZORPAY_KEY) {
        ...
        // Ensure checkout modal vanishes instantly even in no-gateway mode
        this.closeModal();
      ...
      const options = {
        ...
        handler: async (resp)=>{
          ...
          try {
            ...
            await this.finalizeOrder(this._paymentDetails);
          } catch (error) {
    ...
    async finalizeOrder(details){
      ...
      // Persist order summary and redirect immediately
      try { localStorage.setItem('lastOrder', JSON.stringify({ id: order.id, total: order.total })); } catch(_) {}
      this.closeModal();
      window.location.href = 'thanks.html';

      // Best-effort background enrich + sync (non-blocking)
      try {
      ...
      try { setTimeout(()=>{ syncOrdersToSupabase().catch(e=>console.warn('Post-redirect sync failed:', e)); }, 0); } catch(_) {}
    }
    ...
    async flowPickup(orderId){
      ...
      // Prefer fast targeted sync for this order
      try {
        if (window.syncSingleOrder) {
          await window.syncSingleOrder(orderId);
        } else {
