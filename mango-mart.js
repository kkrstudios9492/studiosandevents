    ...
    if (window.supabase) {
        ...
        script.onload = () => {
            if (window.supabase) {
                supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
                supabaseReady = true;
                console.log('Supabase client initialized (loaded)');
            }
        };
        script.onerror = () => {
            console.log('Supabase failed to load, using localStorage fallback');
            supabaseReady = true;
        };
                ...
                <div class="w-full max-w-md">
                    ...
                    <div class="bg-white rounded-lg shadow-lg p-8">
                        ...
                        <form id="signin-form" class="space-y-4">
                            <div>
                                ...
                                <input type="text" id="signin-identifier" required 
                                    class="input" placeholder="your@email.com or 9876543210">
                            </div>
                            <div>
                        ...
                        <div class="mt-6 p-4 bg-blue-50 rounded-lg">
                            <p class="text-sm font-medium text-blue-900 mb-2">Quick Start:</p>
                            <p class="text-xs text-blue-700">
                                Create an account with your email or mobile number!<br>
                                <small>All prices are in Indian Rupees (₹)</small>
                            </p>
                        </div>
                    </div>
                </div>
...
class CustomerDashboard {
    ...
    async updateCartQuantity(productId, quantity) {
        ...
        try {
            ...
            // Reload cart from Supabase
            console.log('Reloading cart from Supabase');
            await this.loadUserCart();
            this.updateCartCount();
    ...
    async placeOrder() {
        ...
        try {
            ...
            // Use the real Supabase order creation instead of mock API
            ...
            if (currentUser) {
                ...
                if (userId) {
                    try {
                        ...
                        console.error('Error clearing Supabase cart:', cartError);
                    }
                }
            }
                        ...
                        <div class="flex justify-center space-x-8">
                            ...
                            <div class="text-center">
                                <div class="w-20 h-20 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-3">
                                    <i data-lucide="star" class="w-10 h-10 text-purple-600"></i>
                                </div>
                                <p class="text-sm font-medium text-gray-700">Quality Products</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-12">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                            ${mainCategories.map(category => {
                                const categoryProducts = this.products.filter(p => category.subcategories.includes(p.category));
                                return `
                    ...
                    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div class="flex flex-col md:flex-row justify-between items-center">
                            ...
                            <div class="flex space-x-6">
                                ...
                                <a href="#" class="text-gray-400 social-facebook transition-colors duration-200" title="Facebook">
                                    <i data-lucide="facebook" class="w-6 h-6"></i>
                                </a>
                                <a href="https://wa.me/919035303173" target="_blank" rel="noopener noreferrer" class="text-gray-400 social-whatsapp transition-colors duration-200" title="Chat with us on WhatsApp">
                                            ...
                                            <div class="p-4">
                                                ...
                                                ` : `
                                                    <div class="flex items-center justify-center space-x-3">
                                                        <button class="quantity-btn w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center" 
    ...
    async showCart() {
        // Load cart from Supabase before showing
        console.log('=== SHOWING CART ===');
        console.log('Before loading cart, this.cart:', this.cart);
        await this.loadUserCart();
        console.log('After loading cart, this.cart:', this.cart);
                ...
                <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    ` : `
                        <div class="mb-4 p-4 bg-yellow-100 border border-yellow-300 rounded">
                            ...
                            <p class="text-yellow-700">Cart length: ${this.cart.length}</p>
                            <p class="text-yellow-700">Cart items: ${JSON.stringify(this.cart, null, 2)}</p>
                        </div>
                        <div class="space-y-4">
                            ${this.cart.map(item => `
    ...
    attachCartEventListeners() {
        ...
        document.querySelectorAll('.quantity-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                console.log('Quantity button clicked:', { id, action, button });
                ...
                if (item) {
                    ...
                    console.log('Item not found in cart:', id, this.cart);
                }
            });
        });
    ...
    handleCheckout() {
        ...
        // Wait a moment for Razorpay to be ready, then initialize payment
        setTimeout(() => {
            if (typeof Razorpay !== 'undefined' && checkRazorpayAvailability()) {
    ...
    async handlePaymentSuccess(response, orderData) {
        ...
        const order = {
            ...
            customer_mobile: session.user.mobile || null, // Allow null for email-only users
            customer_address: orderData.customerAddress,
            customer_landmark: orderData.customerLandmark,
            customer_pincode: orderData.customerPincode,
            items: orderData.items,
    ...
    initiateMockPayment(orderData) {
        ...
        toast.info('Using mock payment for testing. Razorpay is not available.');
        
        // Ensure order has user information
        const session = JSON.parse(localStorage.getItem('session') || 'null');
        if (session && session.user) {
            orderData.customerId = session.user.id;
            orderData.customerName = session.user.name;
            orderData.customerEmail = session.user.email;
            orderData.customerMobile = session.user.mobile;
        }
...
class AdminDashboard {
                            ...
                            ` : `
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    ${this.products.map(product => `
                                        <div class="border border-gray-200 rounded-lg p-4 ${product.stock === 0 || product.status === 'out_of_stock' ? 'opacity-60' : ''}">
                                            ...
                                            <div class="mb-3">
                                                <span class="text-xs ${product.stock > 0 && product.status === 'in_stock' ? 'text-green-600' : 'text-red-600'} font-medium">
                                                    ${product.stock > 0 && product.status === 'in_stock' ? `In Stock (${product.stock || 0})` : 'Out of Stock'}
                                                </span>
                                            </div>
                                            <div class="flex space-x-2">
    ...
    async handleEditProduct() {
        try {
            ...
            const token = this.authManager.getToken();
            await api.updateProduct(updatedProduct.id, updatedProduct, token);
    ...
    async makeAllInStock() {
        ...
        const outOfStockProducts = this.products.filter(p => p.status === 'out_of_stock' || p.stock === 0);
        ...
        try {
            const token = this.authManager.getToken();
            const updatePromises = this.products.map(async (product) => {
                const updatedProduct = {
                    ...product,
                    status: 'in_stock',
                    stock: product.stock === 0 ? 1 : product.stock, // Set to 1 if currently 0, otherwise keep existing stock
                    updatedAt: new Date().toISOString()
                };
                return api.updateProduct(product.id, updatedProduct, token);
            });
            
            await Promise.all(updatePromises);
            
            toast.dismiss(loadingToast);
            toast.success(`Updated ${this.products.length} products to In Stock!`);
...
class MangoMartApp {
    ...
    async init() {
        // Skip initialization on orders page
        if (window.ORDERS_PAGE) {
            console.log('Skipping main app initialization on orders page');
            return;
        }
        ...
        window.createAdminUser = this.createAdminUser.bind(this);
        window.createTestOrder = this.createTestOrder.bind(this);
        window.checkOrdersTable = this.checkOrdersTable.bind(this);
        window.testOrderDisplay = this.testOrderDisplay.bind(this);
        window.testDirectOrderInsert = this.testDirectOrderInsert.bind(this);
        window.debugSupabaseStatus = debugSupabaseStatus;
    }
