                        ...
                        ${mainCategories.map(category => {
                            ...
                            return `
                                <div class="main-category-card bg-white rounded-2xl shadow-xl overflow-hidden cursor-pointer"
                                    ...
                                    <div class="h-full min-h-[160px] bg-gradient-to-br ${category.color} p-4 flex flex-col justify-between text-white relative">
                                        <div class="absolute inset-0 opacity-15">
                                            <div class="absolute top-2 right-2 w-20 h-20 rounded-full bg-white blur-2xl"></div>
                                            <div class="absolute bottom-2 left-2 w-16 h-16 rounded-full bg-white blur-xl"></div>
                                            <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white blur-lg"></div>
                                        </div>
                                        <div class="relative z-10 flex-1 flex flex-col">
                                            <div class="flex items-start justify-between mb-3">
                                                <div class="p-2 bg-white bg-opacity-20 rounded-lg backdrop-blur-sm flex-shrink-0">
                                                    <i data-lucide="${category.icon}" class="w-6 h-6"></i>
                                                </div>
                                                <span class="category-highlight-pill flex-shrink-0">
                                                    Explore
                                                </span>
                                            </div>
                                            <div class="flex-1 flex flex-col">
                                                <h3 class="text-xl font-bold mb-1.5 drop-shadow-lg leading-tight">${category.name}</h3>
                                                <p class="text-white text-opacity-95 mb-3 text-sm font-medium leading-snug flex-1">${category.description}</p>
                                                <div class="flex flex-wrap gap-1 mt-auto">
                                                    ${category.subcategories.slice(0, 3).map(sub => {
                                                    const subcat = sampleCategories.find(c => c.id === sub);
                                                    return subcat ? `
                                                            <span class="text-xs font-medium bg-white bg-opacity-25 backdrop-blur-sm px-2 py-0.5 rounded-full border border-white border-opacity-30 whitespace-nowrap">
                                                            ${subcat.name}
                                                        </span>
                                                    ` : '';
                                                }).join('')}
                                                    ${category.subcategories.length > 3 ? `<span class="text-xs font-medium bg-white bg-opacity-25 backdrop-blur-sm px-2 py-0.5 rounded-full border border-white border-opacity-30">+${category.subcategories.length - 3}</span>` : ''}
                                            </div>
                                        </div>
                                        </div>
                                        <div class="relative z-10 flex justify-end items-center mt-3 pt-3 border-t border-white border-opacity-20">
                                            <div class="p-1.5 bg-white bg-opacity-20 backdrop-blur-sm rounded-full border border-white border-opacity-30 hover:bg-opacity-30 transition-all">
                ...
                <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
                    ...
                    ${relatedProducts.length > 0 ? `
                        <div class="mb-8">
                            ...
                            <div class="relevant-products-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                                    ...
                                    return `
                                        <div class="product-card cursor-pointer bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden" data-product-id="${relatedProduct.id}" data-product-clickable="true">
                                            ...
                                            <div class="w-full space-y-2" data-product-action="${relatedProduct.id}">
                                                ...
                                                ` : relatedQuantity === 0 ? `
                                                    <button class="add-to-cart-btn w-full flex items-center justify-center gap-2 text-sm py-2" data-product-id="${relatedProduct.id}">
                                                        <span class="add-icon">+</span>
                                                        <span>Add</span>
                                                    </button>
                                                ` : `
                                        ...
                                        return `
                                            <div class="product-card h-full flex flex-col cursor-pointer" data-product-id="${product.id}" data-product-clickable="true">
                                                ...
                                                <div class="p-1.5 flex flex-col h-full">
                                                    <h3 class="font-semibold text-xs text-gray-900 mb-0.5">${product.name}</h3>
                                                    <p class="text-gray-600 text-xs mb-1" style="display:-webkit-box;-webkit-line-clamp:1;-webkit-box-orient:vertical;overflow:hidden;">${product.description || ''}</p>
